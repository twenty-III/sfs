...
void RequestParser::parse_query_string(const std::string& query, std::unordered_map<std::string, std::string>& out) {
    ...
    while (getline(ss, key, '=') && getline(ss, value, '&')) out[key] = value;
}
...
bool RequestParser::parse(int client_fd, Request& req) {
    ...
    if (req.method_.empty() || req.path_.empty() || req.version_.empty()) return false;
    ...
    while (std::getline(stream, header_line)) {
        ...
        req.headers_[to_lower_str(key)] = value;
    }

    std::string cl_str = req.get_header("Content-Length");
    if (!cl_str.empty()) {
        size_t content_len = 0;
        try { content_len = std::stoul(cl_str); }
        catch (...) { return false; }

        if (content_len > MAX_BODY_SIZE) return false;
